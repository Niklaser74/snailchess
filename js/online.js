...
export const snigelpost = {
  ...
  // events: the new entries this ply produced; result: { type, winner } when the game ended
  submit(match, events, fen, hp, result) {
    return online.rpc('snailchess_submit', { p_match: match.id, p_ply: match.ply_count + 1, p_events: events, p_fen: fen, p_hp: hp, p_result: result });
  },
  inviteLink(id) {
